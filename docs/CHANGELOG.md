...
- ✨ **Feature:** New option *"Return to Target State After Manual Override Reset"* (`auto_recover_after_manual_override`) in the *Manual Override* section. When enabled, the cover re-evaluates the current situation the moment the manual override is reset (timed or fixed-time reset) and drives to the correct target — sun shading (live re-check of conditions), open, close or ventilation — instead of just clearing the manual flag and waiting for the next scheduled trigger. Mirrors *"Return to Target State After Force Disable"*. Recovery only runs when no force is active and only on *Timed Reset* / *Fixed-Time Reset* (not on *Reset in Position*, which already keeps its existing behavior); otherwise the previous behavior (clear manual flag) is preserved.

  Two important guards keep the recovery cascade safe:

  - **Time-window guard for the OPEN/CLOSE base targets** — without this guard a 04:00 reset after a manual close-for-the-night would drive the cover open in the middle of the night, because with `time_control_disabled` the priority cascade resolves to `bas=opn` 24/7. The OPEN and CLOSE recovery branches are skipped entirely when `time_control_disabled` is set, and only run inside `should_be_open_now` / `should_be_closed_now` windows when time fields or a calendar are configured. The SHADING / LOCKOUT / VENTILATION branches do not need this guard because they already key off real event signals (live shading conditions, window-open contact, window-tilted contact).

  - **User-intent guard on SHADING / OPEN / CLOSE recovery branches** — recovery never moves the cover *back* across the position the user manually set when that means undoing a stronger restriction. Concretely: SHADING recovery only fires if the cover is currently above (lighter than) the shading position; OPEN(base=opn) recovery is skipped when the cover is at or below the close position; CLOSE(base=cls) recovery is skipped when the cover is at or above the open position. Use case: in a shading window you manually drive the cover fully closed because you want it darker than shading — the timeout reset lifts the manual lock but leaves the cover where you put it instead of bouncing it back to the shading position. LOCKOUT and VENTILATION keep their semantics because they handle physical safety (window-open / window-tilted contacts) rather than aesthetic preference.

  - **User-intent guard on the shading-start pending arm and "lift from below" execution** — the same principle is enforced one layer deeper, on the regular shading-start flow (not just the manual-reset recovery cascade). The "Shading detected. Save next execution time and pending status" branch no longer arms `pnd=beg` when the cover is currently below the shading position *and* the latched `bas=opn` is older than the last manual move (i.e. the user manually drove the cover below shading and the stale `bas=opn` is not a legitimate auto-open signal). The "Start Shading" sub-branch's lift-from-below path (`current_below_shading AND effective_state == 'opn'`) carries the same check as defense-in-depth. The legitimate "closed-overnight by automation, lift to shading in the morning" use case still works because the morning `t_open_1` handler refreshes `ts.opn` before the shading pending arms.

  - **Night-sanity floor on `recover_shade`** — the recovery cascade's live shading re-check (`shading_start_conditions_met AND is_shading_allowed_window AND NOT shading_end_conditions_met`) trusts the user's configured shading conditions. When a user has a degenerate `shading_elevation_min` (e.g. `-90°`) the condition can evaluate true at night, so a 23:00 manual open (e.g. for fresh air) followed by the reset timeout would otherwise drive the cover to the shading position. `recover_shade` now additionally requires `current_sun_elevation > 0` as an independent physical sanity floor — sun above the horizon — regardless of how the user configured their shading elevation thresholds. This only applies to the manual-reset recovery path; the regular shading-start flow still respects the user's elevation config end-to-end.

  - **Preserved shading intent across the manual unknown branch** — the *"Manual: position cannot be assigned (unknown)"* branch keeps `shd` and `ts.shd` intact when the recovery option is enabled, so other parts of the automation that read the helper's shading flag (trigger gating, `effective_state` priority cascade) still see the prior intent during the override window. With the option disabled the original behaviour (clear `shd` and refresh `ts.shd`) is preserved as a regression guard for #447.
